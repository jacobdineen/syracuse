...
author: "Copyright 2017, William G. Foote. All rights reserved"
fontsize: 10pt
output:
...
Our company just landed a West Texas Intermediate crude oil contract worth more than 20 percent of our company's current revenue. Now that everyone has recovered from this coup, management wants us to 
...
Here are histories of fuel price returns and their absolute sizes for our viewing pleasure.
...
#head(corr.returns)
plot(corr.returns[,1], ylab = "90 day rolling correlations", xlab = "", main = "Diesel & WTI")
```

***

```{r}
plot(corr.returns[,2], ylab = "90 day rolling correlations", xlab = "", main = "Diesel & Gas")
```

***

```{r}
plot(corr.returns[,3], ylab = "90 day rolling correlations", xlab = "", main = "WTI & Gas")	
```

```{r}
# Run data_moments()
answer <- data_moments(na.omit(corr.returns))
# Build pretty table
answer <- round(answer, 4)
knitr::kable(answer)
```
...
- Build monthly Diesel-WTI correlations and WTI volatilities for WTI
- Regress correlation on volatility to test the hypothesis that WTI volatility might influence Diesel-WTI correlation
...
fisher <- function(r)	
{0.5 * log((1 + r)/(1 - r))}	
rho.fisher <- matrix(fisher(as.numeric(R.corr.vols[,1:6])), nrow = length(WTI.vols), ncol = 6, byrow= FALSE)	
#' 	
#' 	
```
...
```{r}
# Run data_moments()
answer <- data_moments(na.omit(R.corr.vols))
# Build pretty table
answer <- round(answer, 4)
knitr::kable(answer)
```
...
Diesel.WTI.p <- predict(rq(Diesel.corrs ~ Diesel.vols, tau = taus1))
Diesel.WTI.lm.p <- predict(lm(Diesel.corrs ~ Diesel.vols))
colnames(Diesel.WTI.p) <- c(paste("tau",taus1[1]*100, sep = ""), paste("tau",taus1[2]*100, sep = ""), paste("tau",taus1[3]*100, sep = ""))
#head(Diesel.WTI.p)
y <- Diesel.corrs
colnames(Diesel.corrs) <- "y"
str(Diesel.corrs)
Diesel.WTI.CI <- data.frame(WTI.vols = WTI.vols, y = Diesel.corrs, y.5 = Diesel.WTI.p[, 1], y.50 = Diesel.WTI.p[, 2], y.95 = Diesel.WTI.p[, 3], y.lm = Diesel.WTI.lm.p )
str(Diesel.WTI.CI)
#head(Diesel.WTI.CI)
# names(Diesel.WTI.CI)
ggplot(Diesel.WTI.CI, aes(x = WTI.vols, y =  y)) +
    geom_point() +
    ...
    ylab("Diesel.WTI.corrs") +
    ggtitle("Upper and Lower Bounds on Diesel-WTI correlation Relative to WTI Volatility")
```
...
- sometimes large downward movements more likely than upward movements
- Persistent volatility and correlation
- Instability in one market can infect another in the lower end of the correlation distribution
...
VaR.text <- paste("Value at Risk =", round(VaR.hist, 2)) # ="VaR"&c12
ES.text <- paste("Expected Shortfall =", round(ES.hist, 2))
ggplot(loss.rf.df, aes(x = Loss, fill = Distribution)) + geom_density(alpha = 0.8) + geom_vline(aes(xintercept = VaR.hist), linetype = "dashed", size = 1, color = "blue") + geom_vline(aes(xintercept = ES.hist), size = 1, color = "blue") + annotate("text", x = 6, y = 0.2, label = VaR.text) + annotate("text", x = 12, y = 0.1, label = ES.text) + xlim(2, 20) + ggtitle("99% Loss Limits")
```
