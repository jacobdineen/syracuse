...
The project (5) is due before Live Session 10. Submit into **Coursework > Assignments and Grading > Project 5 > Submission** an `RMD`  file 
with filename **lastname_firstname_Project5.Rmd**. If you have difficulties submitting a `.Rmd` file, then submit a `.txt` file. 

1. Use headers (##), r-chunks for code, and text to build a flexdashboard application.
* library(flexdashboard)
* library(shiny)

2. Address the **Optimization and Conclusion portions ONLY** in this file, **changing plots and outputs** as you see fit.
* Optimization
* In Conclusion

2. Complete the calculations for the Conclusion.
...
3. Explain some of the functions used to compute and visualize results.
* List the 'R' skills needed to complete this project

4. Discuss the business implications.
...
With the minimum variance portfolio seen below, we are adding a constraint that w>0, otherwise interpreted as us saying that we are no longer allowing short sales. The minimum variance portfolio is the set of risky assets that, when taken together, result in the lowest possible risk level for the rate of expected return. This type of portfolio hedges each investment with the addition of another investment. The investments in a minimum variance portfolio are individually riskier than the portfolio as a whole (Minimum Variance Portfolio). It is the point at which the red line curves upward into the tangency portfolio which is on the efficient frontier line (Blue line). 

No Constraints:

The weights for the tangency portfolio are as follows:
  
```{r}
weights[ind, ]
```

Go long 178% of aggregate position in Nickel.
Go short 39.8% aggregate position in Copper.
Go short 38% of aggregate position in Aluminum.


The weights for the minimum variance portfolio are as follows
```{r}
weights[ind2,][1,]
```
Long working capital position with only 8% exposure to nickel.
Large exposure to copper and aluminum (45.5% and 46%)
...
sd.R <-  sqrt(diag(cov.R)) ## remember these are in daily percentages
#library(quadprog)
Amat <-  cbind(rep(1,3),mean.R,diag(1,nrow=3))  ## set the equality constraints matrix
mu.P <- seq(min(mean.R)+.0001, max(mean.R)-.0001, length=100)  ## set of 300 possible target portfolio returns
#mu.P <- seq(0.5*quantile_R, max(R), length = 100)  ## set of 300 possible target portfolio returns
sigma.P <-  mu.P ## set up storage for std dev's of portfolio returns
weights <-  matrix(0, nrow=300, ncol = ncol(R)) ## storage for portfolio weights
colnames(weights) <- names.R
...
mu.free <-  .0003 ## input value of daily risk-free interest rate
sharpe <- ( mu.P-mu.free)/sigma.P ## compute Sharpe's ratios
ind5 <-  (sharpe == max(sharpe)) ## Find maximum Sharpe's ratio
ind6 <-  (sigma.P == min(sigma.P)) ## find the minimum variance portfolio
ind7 <-  (mu.P > mu.P[ind6]) ## finally the efficient frontier
col.P <- ifelse(mu.P > mu.P[ind6], "blue", "grey")
sigma.mu.df$col.P <- col.P
renderPlotly({
p <- ggplot(sigma.mu.df, aes(x = sigma.P, y = mu.P, group = 1)) + geom_line(aes(colour=col.P, group = col.P)) + scale_colour_identity() # + xlim(0, max(sd.R*1.1))  + ylim(0, max(mean.R)*1.1) + 
p <- p + geom_point(aes(x = 0, y = mu.free), colour = "red")
...
Long working capital position with 68.2% exposure to Nickel.
Long working capital position with 15.4% exposure to Copper.
Long working capital position with 16.3% exposure to Aluminum.

The weights for the minimum variance portfolio are as follows
```{r}
weights[ind6,]
```
Long working capital position with 23.8% exposure to Nickel.
Long working capital position with 37.8% exposure to Copper.
Long working capital position with 38.4% exposure to Aluminum.
