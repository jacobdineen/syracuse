...
df <- read.csv(file= "/Users/davima10/Documents/Personal/Masters Degree/Class 614/out-201501.csv")[,c(9,12,17,18,23,26,54,59,65,73,76,89:92,106:111,137:147,167:171,175,176,179,182,199:227,232), ]
...
region<-region + geom_map(map=us,fill="orange",color="black") + theme_minimal()
...
dfNPSScoreCalc
## Plot of Bar Chart of Percentage distribution between NPS Types
gNPSTypeDistribution<-ggplot(dfNPSScoreCalc,aes(x=dfNPSScoreCalc$Var1, y=dfNPSScoreCalc$PercentNPSScore,fill=dfNPSScoreCalc$Var1)) + geom_col()+ theme_minimal() +scale_fill_discrete(name='NPS Type')+ ggtitle("Percent of NPS Types") + xlab("NPS Types") + ylab("Percent") #Column Graph Based on Percentage by NPSType
gNPSTypeDistribution
...
##NPS Score by State
dfNPSState<-data.frame(table(dftest$NPSType, dftest$HotelState))
dfNPSState$Freq<-as.numeric(dfNPSState$Freq)
dfNPSState
rownames(dfNPSState) <- NULL
totalSurveyed<-tapply(dfNPSState$Freq,dfNPSState$Var2, sum)
Var2<-rownames(totalSurveyed)
dfTotals<-data.frame(Var2,totalSurveyed)
rownames(dfTotals) <- NULL
dfTotals  
dfNPSState<-merge(x=dfNPSState,y=dfTotals,by="Var2",all=TRUE)
dfNPSState
dfNPSState<-dfNPSState[dfNPSState$Freq!=0,]
rownames(dfNPSState)<-NULL
dfNPSState$PercentNPS<-dfNPSState$Freq / dfNPSState$totalSurveyed
dfNPSState$PercentNPSScore<- dfNPSState$PercentNPS * 100 
dfNPSState<-dfNPSState[dfNPSState$Var1!='Passive',]
dfNPSState$PercentNPSScore<- ifelse(dfNPSState$Var1=='Detractor',dfNPSState$PercentNPSScore*-1,dfNPSState$PercentNPSScore*1)
dfNPSState
NPSByState<-tapply(dfNPSState$PercentNPSScore,dfNPSState$Var2,sum)
NPSByState
dfNPSByState<-data.frame(rownames(NPSByState),NPSByState)
rownames(dfNPSByState)<-NULL
dfNPSByState<-na.omit(dfNPSByState)
rownames(dfNPSByState)<-NULL
dfNPSByState<-dfNPSByState[order(dfNPSByState$NPSByState,decreasing = TRUE),]
colnames(dfNPSByState)<-c('State','NPSScore')
rownames(dfNPSByState)<-NULL
dfNPSByState
gNPSbyState<-ggplot(dfNPSByState,aes(x=dfNPSByState$State, y=dfNPSByState$NPSScore,fill=dfNPSByState$State)) + geom_col(stat="identity") + theme_minimal() + ggtitle("NPS Score by State") + xlab("State") + ylab("Score") +scale_fill_discrete(name="State")#Column Graph Based on NPS Score by State
gNPSbyState

##Likelihood to Recommend Score Breakdown

dfLikelihoodToRe<-data.frame(table(dftest$LikelihoodToReco))
dfLikelihoodToRe
gLikelihoodToRe<-ggplot(dfLikelihoodToRe,aes(x=dfLikelihoodToRe$Var1, y=dfLikelihoodToRe$Freq,fill=dfLikelihoodToRe$Var1)) + geom_col() + theme_minimal() + ggtitle("Score Breakdown") + xlab("Score") + ylab("Number of Scores") +scale_fill_discrete(name="Score")#Column Graph Based on NPS Score by State
gLikelihoodToRe

#Numberline showing how well the NPS score rates
xlim <-c(-100,100)
ylim<-c(0,100)
px<-c(0,50,round(NPSScore),70)
txtpx<-c('Good','Excellent',paste(round(NPSScore), ' - Dataset NPS Score'),'World Class')
py<-c(0,0,0,0)
lx.buf<-25
...
text(lx,ly,txtpx,pos=3)
text(0,50,'NPS Score Levels') ## Levels based on website. "https://www.promoter.io/blog/good-net-promoter-score/"

##View of all NPS Score Charts

grid.arrange(gNPSTypeDistribution,gNPSbyState,gLikelihoodToRe)
...
gBrand<-ggplot(branddf,aes(x=branddf$Var1, y=branddf$Freq, fill=branddf$Var1)) + geom_col() + theme_minimal() +scale_fill_discrete(name="Brand") + ggtitle("Count of Guests per Brand") + xlab("Brand") + ylab("Count") #Column Graph Based on Percentage by NPSType
gBrand
...
gGenderAge<-ggplot(genderage,aes(x=genderage$GuestAgeRange, y=genderage$AVG.LikelihoodToReco.)) + geom_point(color="black",fill="orange") + ggtitle("LTR Score by AgeRange") + xlab("AgeRange") + ylab("Score") #Column Graph Based on Percentage by NPSType
gGenderAge
...
gGender<-ggplot(agedemo,aes(x=agedemo$GuestGender, y=agedemo$percent,fill=agedemo$GuestGender)) + geom_col() + theme_minimal() + scale_fill_discrete(name='Gender') + ggtitle("Gender Split") + xlab("Gender") + ylab("Percent of Whole") #Column Graph Based on Percentage by NPSType
gGender

## Descriptive Charts
grid.arrange(gBrand,gGenderAge,gGender)



#Association Rule Mining
#Attempt Using Audience Demographic Data - May want to do ARM on amenities vs NPS/Likelihood to Reco
...
dfARM <- dfARM[,c(1:9,59)]
dfARM <- na.omit(dfARM)
str(dfARM)

dfARM$Revenue_USD <- as.factor(dfARM$Revenue_USD)
dfARM$NPSType <- as.factor(dfARM$NPSType)

ARM <- apriori(dfARM, parameter = list(support=.5,confidence=.3))
inspect(ARM)                                                    
...
dfSVMS <- dfwscoresfinal[c(10:20,30:59)] 
...
rmse(hotel.testingdata$error) #Computing RMSE. RMSE = .861
...
##Using the KSVM and SVM Models yielded 100% accuracy on Promoter Prediction
...
mapZip <- mapZip + coord_map() + ggtitle("Average Revenue per zip code")
...
mapZipZoomed <-  mapZipZoomed + xlim(merged$longitude-100, merged$longitude+100) + ylim(merged$latitude-100,merged$latitude+100) + coord_map()
